...
if (move_uploaded_file($pic['tmp_name'], $upload_dir_time . $filename)) {
    ...
    foreach ($WidthuploadARR as $K => $V) {
        ...
        $dir_water = str_replace('uploads', 'uploads_water_new', $dir);

        $webp = str_replace('.png', '.webp', $filename);
        $webp = str_replace('.jpeg', '.webp', $webp);
        $webp = str_replace('.jpg', '.webp', $webp);
        $webp = str_replace('.bmp', '.webp', $webp);

//        Сохраняем картинку с вотермаркой для страницы листингов
        $image_w = imagecreatefromjpeg($dir . $filename);
        $watermark = file_exists('watermark'.$V[0].'.png') ? 'watermark'.$V[0].'.png' : 'watermark280.png';
        $watermark300 = imagecreatefrompng($watermark);
        $x = (imagesx($image_w) - imagesx($watermark300)) / 2;
        $y = imagesy($image_w) * 0.6;

        // Налагаем водяной знак на исходное изображение
        imagecopy($image_w, $watermark300, $x, $y, 0, 0, imagesx($watermark300), imagesy($watermark300));

        $water_image = "uploads_water_new/".$V[2]."/undefined/" . $filename;

        imagejpeg($image_w, $water_image, 80);

        exec("cwebp -q 80 ".$water_image." -o "."uploads_water_new/".$V[2]."/undefined/".$webp);
...
//		echo $water_image_460.'<br>';
//		echo $water_image_300.'<br>';
