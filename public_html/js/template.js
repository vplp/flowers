...
$(function() {
    ...
    function get_correct_str(num, str1, str2, str3) {
        var val = eval(num);
        if (val > 10 && val < 20) { return '&nbsp;' + str3; } else {
            val = eval(num) % 10;
            if (val == 1) {
                return '&nbsp;' + str1;
            } else if (val > 1 && val < 5) {
    ...
    function liveSort(callback) {
        ...
        $('.item').each(function() {
            ...
            let sortTypeArr = i_type.split('|');
                ...
                for(let i = 0; i < sortTypeArr.length; i++){
                    ...
                    if(!sortTypeArr[i].toLowerCase().match(searchText) && !searchByName.toLowerCase().match(searchText)){
                        check = false;
                        console.log(searchByName.toLowerCase().match(searchText));
                    }
                }
            ...
            if (price != '') {
                ...
                if ((!priceArr[1] && parseInt(i_price) <= parseInt(price)) || (priceArr[1] && parseInt(i_price) >= parseInt(priceArr[0]) && parseInt(i_price) <= parseInt(priceArr[1]))) {} else {
                    check = false;
                }
