...
<div style="<?php if (Yii::app()->user->getState('edit')) echo 'position:static !important'; ?>"
     class="wrap_block resize_block <?php if ($Cockiesort['smbig'] == 'big') echo 'big_block' ?> <?php if (Yii::app()->user->getState('auth')) echo 'sortable' ?>   <?php if (isset($class)) echo $class; ?>">
    ...
    <?php if (isset($sort) && $sort) :
        ...
        <?php if ($category) { ?>
        <?php if (strip_tags($seoPage['text']) != '') { ?>
                        ...
                        <?php if ($category['id'] != 74) { ?>
                            <div id="sort_type" class="one_sort">
                                ...
                                <div class="one_sort_label_variants">
                                    <a href="/catalog/<?php echo $category['uri']; ?>" class="blue"
                                       data-subtype="flowertype">Все</a>

                                    <?php if ($category['id'] != 73) { ?>
                                        <?php foreach ($flowersInFilter as $flower): ?>
                                            <a href="/catalog/<?php echo $category['uri']; ?>/<?php echo $flower['uri']; ?>"
                                               class="blue <?= ($flower_uri == $flower['uri']) ? 'select' : '' ?>"
                                               data-cat-id="<?php echo $category['id']; ?>"
                                               data-subtype="flowertype"><?php echo $flower['name'] ?></a>
                                        <?php endforeach; ?>
                                    <?php } else { ?>
                        ...
                        <?php if ($_GET['slice'] != $mono_byket['uri']) { ?>
                            <div class="one_sort">
                                ...
                                <?php if (!empty($subcategory_arr) && $category['id'] != 84) { ?>
                                    <div class="one_sort_label"
                                         style="opacity: 0;"><?php echo $category['feature_name'] ?></div>
                                <?php } else { ?>
                                    <div class="one_sort_label"><?php echo $category['feature_name'] ?></div>
                                <?php } ?>

                                <div class="one_sort_label_variants one_sort_type">
                                    <a href="/catalog/<?= !empty($parent_cat['uri']) ? $parent_cat['uri'] : $category['uri'] ?>"
                                       class="blue" data-subtype="flowertype">Все</a>

                                    <?php if (!empty($subcategory_arr)) { ?>
                                        <?php foreach ($subcategory_arr as $item): ?>
                                            <?php if ($item['count_products'] > 0) { ?>
                                                <a href="/catalog/<?php echo $item['uri'] ?>"
                                                   class="blue <?= ($_GET['uri'] == $item['uri']) ? 'select' : '' ?>"
                                                   data-subtype="noflowertype"><?php echo $item['name'] ?></a>
                                            <?php } ?>
                                        <?php endforeach; ?>
                                    ...
                                    <a href="/catalog/byketi/nazakaz"
                                       class="blue hide-filter-item <?= ($_GET['slice'] == "nazakaz") ? 'select' : '' ?>"
                                       data-subtype="flowertype">На заказ</a>
                                    <a href="/catalog/byketi/gotovie"
                                       class="blue hide-filter-item <?= ($_GET['slice'] == "gotovie") ? 'select' : '' ?>"
                                       data-subtype="flowertype">В наличии</a>
...
//                        die();
                    ...
                    <div id="sort_status" class="one_sort hide-filter-item">
                        <!--                            <a class="blue -->
                        <? //= ($_GET['slice'] == "nazakaz") ? 'select' : '' ?><!--">На заказ</a>-->
                        <!--                            <a class="blue -->
                        <? //= ($_GET['slice'] == "gotovie") ? 'select' : '' ?><!--">В наличии</a>-->
                    </div>
                    ...
                    <div id="sort_price" class="one_sort">
                        ...
                        <div class="one_sort_label_variants price">
                            ...
                            <?php foreach ($priceRanges as $priceRange) { ?>
                                <a href="" data-price="<?php echo $priceRange['prices'] ?>"
                                   class="blue "><?php echo $priceRange['title'] ?> <span class="b-rub">Р</span></a>
                            <?php } ?>
                            <!--
                ...
                <div id="sort_price" class="one_sort sort_name">
                    ...
                    <div class="one_sort_label_variants price">
                        <form class="searchByName">
                            <input name="searchByName" type="text" placeholder="Ввести название" maxlength="50"
                                   autocomplete="off">
                        </form>
                    </div>
                </div>
